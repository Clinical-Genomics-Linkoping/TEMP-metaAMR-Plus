...
workflow POLISH_ASSEMBLY {
    ...
    // **Ensure input assemblies are in .gz format**
    ch_prepped_assembly = assembly.map { meta, assembly_file ->
        def final_assembly = file("${meta.id}.assembly.fasta.gz")

        if (!assembly_file.name.endsWith(".gz")) {
            assembly_file.copyTo(file("${meta.id}.assembly.fasta"))
            "gzip -f ${meta.id}.assembly.fasta".execute().waitFor()
        } else {
    ...
    ch_racon1_gzipped = RACON_1.out.improved_assembly.map { meta, racon1_fasta ->
        def racon1_gz = file("${meta.id}.racon1.fasta.gz")

        if (!racon1_fasta.name.endsWith(".gz")) {
            racon1_fasta.copyTo(file("${meta.id}.racon1.fasta"))
            "gzip -f ${meta.id}.racon1.fasta".execute().waitFor()
        } else {
    ...
    ch_racon2_gzipped = RACON_2.out.improved_assembly.map { meta, racon2_fasta ->
        def racon2_gz = file("${meta.id}.racon2.fasta.gz")

        if (!racon2_fasta.name.endsWith(".gz")) {
            racon2_fasta.copyTo(file("${meta.id}.racon2.fasta"))
            "gzip -f ${meta.id}.racon2.fasta".execute().waitFor()
        } else {
