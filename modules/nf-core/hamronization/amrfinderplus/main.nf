/*process HAMRONIZATION_AMRFINDERPLUS {
    ...
    """
    # Check if the output file already exists and remove it to prevent conflicts
    if [ -f "${prefix}.${format}" ]; then
        echo "Output file ${prefix}.${format} already exists. Removing it."
        rm -f "${prefix}.${format}"
    fi
    
    echo "Debug: Running hamronize with the following command:"
    echo "hamronize \\
    amrfinderplus \\
    ${report} \\
    $args \\
